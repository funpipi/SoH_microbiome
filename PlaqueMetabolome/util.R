...
#--------------------------------------------------
BetweenGroup.test <-function(data, group, p.adj.method="fdr",paired=FALSE, p_adj_cutoff=0.05){
# p.adjust.methods
...
if(n_group==2){
               ...
               colnames(output1)<-c(paste("mean_",levels(group)[1],sep=""),paste("mean_",levels(group)[2],sep=""), "Association",
                                   paste("sd_",levels(group)[1],sep=""),paste("sd_",levels(group)[2],sep=""),"T-test","Wilcoxon.test",
                                   paste("T-test_",p.adj.method,sep=""),paste("Wilcoxon.test_",p.adj.method,sep=""))
               for(i in 1:ncol(data))
               {
                 if(var(data[, i])==0){
                   output1[i,]<-c(rep(0, 4),rep(1, 4))
                 }else{
               output1[i,1]<-mean(data[which(group==levels(group)[1]),i])
               output1[i,2]<-mean(data[which(group==levels(group)[2]),i])
               output1[i,3]<-NA
               output1[i,4]<-sd(data[which(group==levels(group)[1]),i])
               output1[i,5]<-sd(data[which(group==levels(group)[2]),i])
               ...
               output1[,3]<-apply(output1, 1, function(x) { 
                 if(x[1]>x[2]){ x[3]<-paste(levels(group)[1], "-enriched", sep="")
                 }else if(x[1]<x[2]){ x[3]<-paste(levels(group)[1], "-depleted", sep="")
                }})
               SigID<-which(output1[,ncol(output1)]>p_adj_cutoff)
               output1[SigID,3]<-"Neutral"
               
               AUCs<-apply(data,2,function(x) auc(group,x)); AUCs[AUCs<0.5]<-1-AUCs[AUCs<0.5]
               AllOccRate<-t(apply(data,2,function(x) tapply(x, group,function(x) sum(x!=0)/length(x)))); colnames(AllOccRate)<-paste("OccRate",colnames(AllOccRate),sep="_");AllOccRate<-AllOccRate[order(rownames(AllOccRate)),]
               AllMedianAbund=t(apply(data,2,function(x) tapply(x,group,median))); colnames(AllMedianAbund)<-paste("Median",colnames(AllMedianAbund),sep="_");AllMedianAbund<-AllMedianAbund[order(rownames(AllMedianAbund)),]
               ...
               output1<-data.frame(AUCs, AllOccRate, AllMedianAbund, AllMedianAbundLog10, output1)
